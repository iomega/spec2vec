...
def test_calc_vector_higher_than_allowed_missing_fraction():
    """Test using a pretrained network and a missing word fraction above allowed."""
...
def test_calc_vector_within_allowed_missing_fraction():
    ...
    document = SpectrumDocument(spectrum, n_decimals=1)
    model = import_pretrained_model()
    vector = calc_vector(model, document, intensity_weighting_power=0.5, allowed_missing_fraction=20.0)
